...
BEGIN
    ...
    LOOP
        ...
        IF NOT FOUND THEN
            ...
            result_items := result_items || (position_record.produkt_name || ': Produkt nicht im Lager gefunden');
            CONTINUE;
        END IF;
        
        -- Prüfe ob genug Lagerbestand vorhanden
        IF current_stock < position_record.menge THEN
            result_items := result_items || (position_record.produkt_name || ': Nicht genug Lagerbestand (verfügbar: ' || current_stock || ', benötigt: ' || position_record.menge || ')');
            CONTINUE;
        END IF;
        ...
        result_items := result_items || (position_record.produkt_name || ': ' || position_record.menge || ' reduziert, neuer Bestand: ' || (current_stock - position_record.menge));
    END LOOP;
...
-- Trigger zum automatischen Erstellen von Bestellpositionen (korrigiert)
...
BEGIN
    ...
    FOR item IN SELECT * FROM jsonb_array_elements(NEW.bestellung_json)
    LOOP
